...
using System.Linq;
using TCPingInfoViewLib.Comparer;
...
namespace TCPingInfoViewLib.Utils
{
	public static class VersionUtil
	{
		...
		{
			str = RemoveOldVersionChar(str);
			return Version.TryParse(str, out _);
		}

		private static string RemoveOldVersionChar(string str)
		{
			return str.Replace(@"v", string.Empty).Replace(@"-steam", string.Empty);
		}
		...
		{
			//Remove old version char
			v1 = RemoveOldVersionChar(v1);
			v2 = RemoveOldVersionChar(v2);
